/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SurveyLinkWhereInput } from "./SurveyLinkWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SurveyLinkOrderByInput } from "./SurveyLinkOrderByInput";

@ArgsType()
class SurveyLinkFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SurveyLinkWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SurveyLinkWhereInput, { nullable: true })
  @Type(() => SurveyLinkWhereInput)
  where?: SurveyLinkWhereInput;

  @ApiProperty({
    required: false,
    type: [SurveyLinkOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SurveyLinkOrderByInput], { nullable: true })
  @Type(() => SurveyLinkOrderByInput)
  orderBy?: Array<SurveyLinkOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SurveyLinkFindManyArgs as SurveyLinkFindManyArgs };
